...
G-7 gross domestic expenditure on R&D (% GDP) data
```{r}
...
Call libraries for liner regression modelling 
```{r}
library(GGally)
library(ggfortify)
library(modelr)
...
```{r}
model2 <- lm(gdp_phw_usd ~ gdexp_rnd_bil_usd + rail_bil_eur, data = prod_uk_trim)

autoplot(model2) #diagnostics
```
...
Adding a third predictor to the model
```{r}
model3 <- lm(gdp_phw_usd ~ gdexp_rnd_bil_usd + rail_bil_eur + inland_bil_usd, data = prod_uk_trim)

autoplot(model3) #diagnostics
```
...
```{r}
model4 <- lm(gdp_phw_usd ~ gdexp_rnd_bil_usd + rail_bil_eur + inland_bil_usd + road_bil_eur, data = prod_uk_trim)

autoplot(model4) #diagnostics
```
...
Check for including in the model  potential interactions between predictors
```{r}
coplot(gdp_phw_usd ~ road_bil_eur | inland_bil_usd,
       rows = 1,
       panel = function(x, y, ...){
         points(x, y)
         abline(lm(y ~ x), col = "blue")
...
```{r}
model5 <- lm(gdp_phw_usd ~ gdexp_rnd_bil_usd + rail_bil_eur + inland_bil_usd + road_bil_eur + road_bil_eur:inland_bil_usd, data = prod_uk_trim)

autoplot(model5) #diagnostics
```
...
Model :
gdp_phw_usd ~ gdexp_rnd_bil_usd + rail_bil_eur + road_bil_eur + 
    inland_bil_usd
