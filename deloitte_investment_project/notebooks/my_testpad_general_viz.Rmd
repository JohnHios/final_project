...
International_Labour_Productivity-G7
...
Change Year in datetime format
```{r}
gdp_phw_current <- gdp_phw_current %>%
  mutate(Year = make_datetime(Year)) 

gdp_pw_current <- gdp_pw_current %>%
  mutate(Year = make_datetime(Year)) 

gdp_phw_const <- gdp_phw_const %>%
  mutate(Year = make_datetime(Year)) 

gdp_pw_const <- gdp_pw_const %>%
  mutate(Year = make_datetime(Year)) 
```
...
tidy_gdp_phw_const %>% 
  ...
  labs(
    x = "\n Year", 
    y = "GDP per hour worked (constant)\n"
  ) 
```
...
Labour_Productivity-OECD_Countries
...
gdp_phw_OECD %>% 
  ...
  ggtitle(
    "GDP per hour worked",
    subtitle =  "indicator is measured in USD (index 2010 = 100)"
  ) +
  labs(
    x = "\n Year", 
    y = "GDP per hour worked ($) \n"
...
########################################################################################################################
France has a similar GDP with the UK for the past years and the population is similar, hence it is ideal for making one-to-one comparisons.
Overall, France is doing better than the UK in terms of productivity (GDP per hrs worked or GDP per worker) 
```{r}
productivity_phw %>% 
  select(`A*10 (excl L)`, `NACE Industry`,  `France`, `United Kingdom`) %>% 
  mutate(
    France = 100*France/sum(France),
    `United Kingdom` = 100*`United Kingdom`/sum(`United Kingdom`),
  ) %>% 
  ggplot() +
  aes(
    x = France, 
  ...
  geom_text(
    aes(label = `A*10 (excl L)`),
    nudge_x = 0.75,
    nudge_y = 0,
    size = 3.5
  ) +
  ggtitle(
    "Differences in industrial output structure",
    subtitle =  "(Proportion of Output per hour by industry (%);  Years: 2014 - 2016)"
  ) +
  xlim(0, 25) +
  ylim(0, 25)
```
...
```{r}
productivity_phw %>% 
  select(`A*10 (excl L)`, `NACE Industry`,  `Germany`, `United Kingdom`) %>% 
  mutate(
    Germany = 100*Germany/sum(Germany),
    `United Kingdom` = 100*`United Kingdom`/sum(`United Kingdom`),
  ) %>% 
  ggplot() +
  aes(
...
```{r}
productivity_phw %>% 
  select(`A*10 (excl L)`, `NACE Industry`,  `Spain`, `United Kingdom`) %>% 
  mutate(
    Spain = 100*Spain/sum(Spain),
    `United Kingdom` = 100*`United Kingdom`/sum(`United Kingdom`),
  ) %>%
  ggplot() +
  aes(
...
```{r}
productivity_phw %>% 
  select(`A*10 (excl L)`, `NACE Industry`,  `Italy`, `United Kingdom`) %>% 
  mutate(
    Italy = 100*Italy/sum(Italy),
    `United Kingdom` = 100*`United Kingdom`/sum(`United Kingdom`),
  ) %>%
  ggplot() +
  aes(
...
Comparison with most productive countries in Europe for 2017: Ireland, Norway
Output per hour by industry 2014-16


```{r}
productivity_phw %>% 
  select(`United Kingdom`, Ireland, Norway) %>% 
  summarise(
    sum_UK = sum(`United Kingdom`),
    sum_Ireland = sum(Ireland),
    sum_Norway = sum(Norway)
  )
...
```{r}
productivity_phw %>% 
  select(`A*10 (excl L)`, `NACE Industry`,  `Ireland`, `United Kingdom`) %>% 
  mutate(
    Ireland = 100*Ireland/sum(Ireland),
    `United Kingdom` = 100*`United Kingdom`/sum(`United Kingdom`),
  ) %>%
  ggplot() +
  aes(
...
```{r}
productivity_phw %>% 
  select(`A*10 (excl L)`, `NACE Industry`,  `Norway`, `United Kingdom`) %>% 
  mutate(
    Norway = 100*Norway/sum(Norway),
    `United Kingdom` = 100*`United Kingdom`/sum(`United Kingdom`),
  ) %>%
  ggplot() +
  aes(
...
gdexp_rd %>%
  ...
  ggtitle(
    "Cross country gross domestic expenditure on R&D",
    subtitle =  "(Year 2016)"
  ) +
  labs(
    x = "\n Country", 
...
domestic_credit_to_private_sector %>%
  ...
  ggtitle("G-7 domestic credit to private sector") +
  labs(
    x = "Country", 
    y = "GDP (%)"
  ) 
...
adult_education %>%
  ...
  # scale_colour_manual(values = cbb_palette) +
  ggtitle(
    "G-7 countries adult education levels",
    subtitle = "Age group: 25-64 years"
  ) +
  labs(
    x = "\n Year", 
    y = "Same age group population (%) \n"
  ) 
```
...
adult_education %>%
 mutate(TIME = year(TIME)) %>% 
  ...
  ggtitle(
    ...
    subtitle = "Age group: 25-64; Year: 2017"
  ) +
  labs(
    x = "\n Country", 
    y = "Same age group population (%) \n"
...
OECD Quality of human capital - Better to combine the data sets from the 3 sheets in file below
...
tidy_overqualified_workers %>%
  ...
  scale_colour_brewer(palette = "Dark2") +
  # scale_colour_manual(values = cbb_palette) +
  ggtitle("G-7 countries percentage of overqualified workers") +
  labs(
    x = "\n Year", 
    y = "Percentage (%) \n"
  ) 
...
tidy_underqualified_workers %>%
  ...
  scale_colour_brewer(palette = "Dark2") +
  # scale_colour_manual(values = cbb_palette) +
  ggtitle("G-7 countries percentage of underqualified workers") +
  labs(
    x = "\n Year", 
    y = "Percentage (%) \n"
  ) 
...
tidy_qualified_workers %>%
  ...
  scale_colour_brewer(palette = "Dark2") +
  # scale_colour_manual(values = cbb_palette) +
  ggtitle("G-7 countries percentage of well-matched workers") +
  labs(
    x = "\n Year", 
    y = "Percentage (%) \n"
  ) 
...
tidy_overqualified_workers %>%
  ...
  ggtitle(
    "Percentage of overqualified workers",
    subtitle = "(Year 2013)"
  ) +
  labs(
    x = "\n Country", 
...
tidy_underqualified_workers %>%
  ...
  ggtitle(
    "Percentage of underqualified workers",
    subtitle = "(Year 2013)"
  ) +
  labs(
    x = "\n Country", 
...
tidy_qualified_workers %>%
  ...
  ggtitle(
    "Percentage of well-matched workers",
    subtitle = "(Year 2013)"
  ) +
  labs(
    x = "\n Country", 
...
education_expenditure %>%
  ...
  ggtitle(
    ...
    subtitle = "Tertiary education (Ye)"
  ) +
  labs(
    x = "\n Country", 
    y = "GDP (%) \n"
...
__Multifactor productivity__


Read data
```{r}
mfp <- read_excel(here("raw_data/Multifactor_Productivity-OECD_Countries.xls")) 
...
mfp %>% 
  # distinct(LOCATION)
   distinct(INDICATOR) # MFP
  # distinct(SUBJECT) # TOT
  # distinct(MEASURE) # AGRWTH, IDX2010
  # distinct(FREQUENCY) #A
  # distinct(`Flag Codes`) # NA
```

G-7 countries Multifactor productivity (annual growth rate)
```{r}
mfp%>% 
  select(-c(`Flag Codes`, INDICATOR, FREQUENCY, SUBJECT)) %>% 
  filter(
  ...
  # scale_colour_manual(values = cbb_palette) +
  ggtitle(
    "G-7 countries Multifactor productivity",
...
G-7 countries Multifactor productivity (index 2010)
```{r}
mfp%>% 
  select(-c(`Flag Codes`, INDICATOR, FREQUENCY, SUBJECT)) %>% 
  filter(
